fn main() {
    ...
    let initial_json = manifest_path.join("outputs/delegated_proving_test.initial.json");
    ...
    if initial_json.exists() {
        println!("cargo:rerun-if-changed=outputs/delegated_proving_test.initial.json");
    }
