fn main() {
    ...
    let build_aleo = manifest_path.join("build/delegated_proving_test/delegated_proving_test.aleo");
    let build_abi = manifest_path.join("build/delegated_proving_test/abi.json");

    if src_main_leo.exists() && src_main_aleo.exists() {
        panic!("Cannot have both src/main.leo and src/main.aleo; remove one.");
    }

    println!("cargo:rerun-if-changed=build/delegated_proving_test/abi.json");
