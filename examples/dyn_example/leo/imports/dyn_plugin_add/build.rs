fn main() {
    ...
    let build_aleo = manifest_path.join("build/dyn_plugin_add/dyn_plugin_add.aleo");
    let build_abi = manifest_path.join("build/dyn_plugin_add/abi.json");

    if src_main_leo.exists() && src_main_aleo.exists() {
        panic!("Cannot have both src/main.leo and src/main.aleo; remove one.");
    }

    println!("cargo:rerun-if-changed=build/dyn_plugin_add/abi.json");
