...
impl syn::parse::Parse for MacroArgs {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        ...
        input.parse::<Token![,]>().expect("Expected two arguments");
        let network = input.parse().expect("Expected a network");
...
pub fn generate_bindings(input: TokenStream) -> TokenStream {
    ...
    let json_content =
        std::fs::read_to_string(json_path.value()).expect("Failed to read JSON file");

    let signatures_json = get_signatures(&json_content).expect("Failed to extract signatures");

    let simplified: SimplifiedBindings =
        serde_json::from_str(&signatures_json).expect("Failed to parse simplified JSON");
